...
public class PropertyBasedPropagator implements Propagator{
	...
	public Set<Match> propagate(Matcher matcher, Match match, Set<Function> unmatchedSrcFuncSet,
			Set<Function> unmatchedDstFuncSet, TaskMonitor monitor) throws CancelledException {

		Function srcMatchedFunction = match.getSourceFunction();
		Function dstMatchedFunction = match.getDestinationFunction();

		Set<Function> limitedUnmatchedSrcFuncSet = srcProperty.getPropagatedFuncs(srcMatchedFunction,
				unmatchedSrcFuncSet);
		Set<Function> limitedUnmatchedDstFuncSet = dstProperty.getPropagatedFuncs(dstMatchedFunction,
				unmatchedDstFuncSet);

		Set<Match> matches =  matcher.doMatch(limitedUnmatchedSrcFuncSet, limitedUnmatchedDstFuncSet, monitor);
