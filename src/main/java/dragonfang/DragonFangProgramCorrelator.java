...
public class DragonFangProgramCorrelator extends VTAbstractProgramCorrelator {
	...
	public DragonFangProgramCorrelator(ServiceProvider serviceProvider, Program sourceProgram,
			AddressSetView sourceAddressSet, Program destinationProgram, AddressSetView destinationAddressSet,
			ToolOptions options, String name, DragonFangData dragonFangData) {
	 ...
	 */
	private Set<Function> getFunctionSet(Program program, AddressSetView asv, TaskMonitor monitor) {

		Set<Function> functionSet = new HashSet<Function>();

		FunctionIterator funcIter = program.getFunctionManager().getFunctionsNoStubs(asv, true);
	...
	private VTMatchInfo createMatchInfo(VTMatchSet matchSet, Function sourceFunction, Function destinationFunction,
			VTScore similarity, VTScore confidence, VTMatchTag tag) {
	...
	private void processNewMatches(Set<Match> foundMatches, VTMatchSet matchSet, Set<Function> unmatchedSrcFuncSet,
			Set<Function> unmatchedDstFuncSet, Queue<Match> propagationQueue) {
		...
		for (Match match : foundMatches) {
			...
			VTMatchTag tag = dragonFangData.getMatchTagAssigner().assignTag(similarity, reason);
			VTMatchInfo matchInfo = createMatchInfo(matchSet, srcFunction, dstFunction, new VTScore(similarity),
					new VTScore(confidence), tag);
	...
	protected void doCorrelate(VTMatchSet matchSet, TaskMonitor monitor) throws CancelledException {
		...
		Set<Function> unmatchedSrcFuncSet = getFunctionSet(getSourceProgram(), getSourceAddressSet(), monitor);
		Set<Function> unmatchedDstFuncSet = getFunctionSet(getDestinationProgram(), getDestinationAddressSet(),
				monitor);

		boolean doPropagation = getOptions().getBoolean(DragonFangProgramCorrelatorFactory.PROPAGATION_STEP_OPT,
		...
		for (Matcher matcher : matchers) {
			...
			Set<Match> initialMatches = matcher.doMatch(unmatchedSrcFuncSet, unmatchedDstFuncSet, monitor);
			processNewMatches(initialMatches, matchSet, unmatchedSrcFuncSet, unmatchedDstFuncSet, propagationQueue);
			...
			if (doPropagation) {
				while (!propagationQueue.isEmpty() && !unmatchedSrcFuncSet.isEmpty()
					...
					for (Propagator propagator : propagators) {
						Set<Match> newMatches = propagator.propagate(matcher, match, unmatchedSrcFuncSet,
								unmatchedDstFuncSet, monitor);
						processNewMatches(newMatches, matchSet, unmatchedSrcFuncSet, unmatchedDstFuncSet,
								propagationQueue);
					}
				}
			}
