...
public class SimilarityFeatureMatcher extends AbstractFeatureMatcher {
	...
	@Override
	public Set<Match> doMatch(Set<Function> unmatchedSrcFuncSet, Set<Function> unmatchedDstFuncSet, TaskMonitor monitor)
			throws CancelledException {

		Set<Match> matches = new HashSet<Match>();

		HashMap<FeatureVector, List<Function>> srcMatchMap = deriveMatchMap(unmatchedSrcFuncSet, srcFeatureMap,
				monitor);
		HashMap<FeatureVector, List<Function>> dstMatchMap = deriveMatchMap(unmatchedDstFuncSet, dstFeatureMap,
				monitor);
		...
		for (Map.Entry<FeatureVector, List<Function>> srcEntry : srcMatchMap.entrySet()) {
			...
			if (srcFuncList.size() == 1) {
				...
				for (Map.Entry<FeatureVector, List<Function>> dstEntry : dstMatchMap.entrySet()) {
					FeatureVector dstFeatureVector = dstEntry.getKey();
					List<Function> dstFuncList = dstEntry.getValue();

					double similarity = similarityMetric.calculateSimilarity(srcFeatureVector, dstFeatureVector);
				...
				if (bestDstFuncList != null && bestDstFuncList.size() == 1) {
					...
					Match match = new Match(srcFuncList.get(0), bestDstFuncList.get(0), bestSimilarity, confidence, "Similarity Feature Matcher");
					matches.add(match);
				}
			}
		}
