...
public class UniqueFeatureMatcher extends AbstractFeatureMatcher {
	...
	public Set<Match> doMatch(Set<Function> unmatchedSrcFuncSet, Set<Function> unmatchedDstFuncSet, TaskMonitor monitor)
		...
		for (Function unmatchedSrcFunction : unmatchedSrcFuncSet) {
			FeatureVector featureVector = srcFeatureMap.getFeature(unmatchedSrcFunction, monitor);
			...
			if (srcFuncList.size() == 1) {
				...
				if (dstFuncList != null && dstFuncList.size() == 1) {
					...
					Match match = new Match(unmatchedSrcFunction, dstFuncList.get(0), similarity, confidence,
