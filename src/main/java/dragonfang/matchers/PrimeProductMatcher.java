...
public class PrimeProductMatcher extends AbstractPrimeProductMatcher {
	...
	public Set<Match> doMatch(Set<Function> unmatchedSrcFuncSet, Set<Function> unmatchedDstFuncSet, TaskMonitor monitor)
		...
		HashMap<Long, List<Function>> srcMatchMap = deriveMatchMap(unmatchedSrcFuncSet, srcPrimeProductMap, monitor);
		HashMap<Long, List<Function>> dstMatchMap = deriveMatchMap(unmatchedDstFuncSet, dstPrimeProductMap, monitor);

		for (Function unmatchedSrcFunction : unmatchedSrcFuncSet) {
			Long primeProduct = srcPrimeProductMap.getPrimeProduct(unmatchedSrcFunction, monitor);
			...
			if (srcFuncList.size() == 1) {
				...
				if (dstFuncList != null && dstFuncList.size() == 1) {
					...
					Match match = new Match(unmatchedSrcFunction, dstFuncList.get(0), similarity, confidence, "Prime Product Matcher");
					matches.add(match);
				}
			}
		}
