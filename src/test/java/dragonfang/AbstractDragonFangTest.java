...
public abstract class AbstractDragonFangTest extends AbstractGhidraHeadlessIntegrationTest {
	...
	private ProgramBuilder initProgBuilder() throws Exception {
		...
		ProgramBuilder progBuilder = new ProgramBuilder("sample", ProgramBuilder._X86, null);
	...
	private void buildSimpleFunction(ProgramBuilder progBuilder) throws Exception {
		...
		progBuilder.setBytes("0x1006420",
		...
		entry.setBody(new AddressSet(program, addr(progBuilder, "0x1006420"), addr(progBuilder, "0x100646E")));
		prog.endTransaction(transID, true);

		progBuilder
				.disassemble(new AddressSet(program, addr(progBuilder, "0x1006420"), addr(progBuilder, "0x010065aa")));
